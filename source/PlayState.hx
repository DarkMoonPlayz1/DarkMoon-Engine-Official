...
class PlayState extends MusicBeatState
{
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			else if (FlxG.save.data.Circles)
			{
				babyArrow.frames = Paths.getSparrowAtlas('notes/circle_assets');
				for (j in 0...4)
				{
					babyArrow.animation.addByPrefix(dataColor[j], 'arrow' + dataSuffix[j]);
					babyArrow.animation.addByPrefix('dirCon' + j, dataSuffix[j].toLowerCase() + ' confirm', 24, false);
	...
	override public function update(elapsed:Float)
	{
		...
		{
			if (unspawnNotes[0].strumTime - Conductor.songPosition < 15000 * songMultiplier)
			{
		...
		if (!PlayStateChangeables.Optimize)
			if (FlxG.keys.justPressed.EIGHT && songStarted)
			{
				...
				FlxG.switchState(new StageEditorState(Stage.curStage, gf.curCharacter, boyfriend.curCharacter, dad.curCharacter));
				clean();
				FlxG.stage.removeEventListener(KeyboardEvent.KEY_DOWN, handleInput);
				FlxG.stage.removeEventListener(KeyboardEvent.KEY_UP, releaseInput);
				#if FEATURE_LUAMODCHART
					...
					else
					{
						if (loadRep && daNote.isSustainNote)
						{
							...
							else
							{
								...
								if (daNote.isParent)
								{
									health -= 0.07; // give a health punishment for failing a LN
	...
	function endSong():Void
	{
		...
		else if (stageTesting)
		{
			...
			FlxG.switchState(new StageEditorState(Stage.curStage));
		}
		else
		{
			if (isStoryMode)
